...
```sh
curl -vu mytesting:mytesting 'http://localhost:9191/api/oauth/token?username=admin&password=admin&grant_type=password'
...
```sh
curl -vu mytesting:mytesting 'http://localhost:9191/api/oauth/token?grant_type=refresh_token&refresh_token=<refresh_token>'
