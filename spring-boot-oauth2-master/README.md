...
```sh
curl -vu rajithapp:secret "http://localhost:8080/api/oauth/token?username=admin&password=admin&grant_type=password"
...
```sh
curl -vu rajithapp:secret 'http://localhost:8080/api/oauth/token?grant_type=refresh_token&refresh_token=<refresh_token>'
...
```sh
curl -i -H "Authorization: Bearer 821c99d4-2c9f-4990-b68d-18eacaff54b2" http://localhost:8080/api/secure
